...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<CreatorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/creators` : null),
    () => getCreators({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Creator',
      path: isTenantUser ? `/creators/view/${data?.[0]?.id}` : '/creators',
      entity: 'creator',
      service: AccessServiceEnum.PROJECT,
      icon: FiUserPlus,
    },
    { name: 'Campaign', path: '/campaigns', entity: 'campaign', service: AccessServiceEnum.PROJECT, icon: FiFlag },
    {
      name: 'Collaborator',
      path: '/collaborators',
      entity: 'collaborator',
